...
const char kSetWindowBottomMode[] = "setWindowBottomMode";
const char kSetWindowFullscreenMode[] = "setWindowFullscreenMode";
...
// Gets the display connection./home/vladimir/Desktop/AIS_voter/AIS_voting/src/clients/deputy/build/linux/x64/release/bundle
...
// Sets the window on bottom of screen, sets AlwaysOnTop = true
static FlMethodResponse* set_window_bottom_mode(FlWindowSizePlugin* self,
                                                 FlValue* args) {
  GtkWindow* window = get_window(self);
  if (window == nullptr) {
    return FL_METHOD_RESPONSE(
  ...
  gtk_window_move(GTK_WINDOW(window), 0, workarea.height - 60);

  self->window_geometry.max_width = static_cast<gint>(workarea.width);
  self->window_geometry.max_height = static_cast<gint>(60);
  self->window_geometry.min_width = static_cast<gint>(workarea.width);
  self->window_geometry.min_height = static_cast<gint>(60);

  update_window_geometry(self);

  gtk_window_resize(window, static_cast<gint>(workarea.width),
                static_cast<gint>(60));
...
// Sets the window fullscreen, sets AlwaysOnTop = false
static FlMethodResponse* set_window_fullscreen_mode(FlWindowSizePlugin* self,
                                                 FlValue* args) {
  ...
  self->window_geometry.max_width = static_cast<gint>(workarea.width);
  self->window_geometry.max_height = static_cast<gint>(workarea.height);
  self->window_geometry.min_width = static_cast<gint>(workarea.width);
  self->window_geometry.min_height = static_cast<gint>(workarea.height);
  
  update_window_geometry(self);

  gtk_window_move(GTK_WINDOW(window), 0, 0);
  
  gtk_window_resize(GTK_WINDOW(window), static_cast<gint>(workarea.width),
                    static_cast<gint>(workarea.height));
...
static void method_call_cb(FlMethodChannel* channel, FlMethodCall* method_call,
  ...
  } else if (strcmp(method, kSetWindowBottomMode) == 0) {
    response = set_window_bottom_mode(self, args);
  } else if (strcmp(method, kSetWindowFullscreenMode) == 0) {
    response = set_window_fullscreen_mode(self, args);
